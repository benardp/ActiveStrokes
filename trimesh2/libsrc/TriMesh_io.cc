...
static bool write_objn(TriMesh *mesh, FILE *f);
...
static bool write_normals_asc(TriMesh *mesh, FILE *f,
			    const char *before_norm,
			    const char *after_line);
static bool write_faces_asc(TriMesh *mesh, FILE *f,
			    const char *before_face, const char *after_line);
static bool write_faces_normals_asc(TriMesh *mesh, FILE *f,
			    const char *before_face, const char *after_line);
...
bool TriMesh::write(const char *filename)
{
	...
	enum { PLY_ASCII, PLY_BINARY_BE, PLY_BINARY_LE,
	       RAY, OBJ, OBJN, OFF, SM, CC } filetype;
	...
	while (1) {
		if (!strncasecmp(filename, "norm:", 5)) {
			...
			filename += 4;
			filetype = OBJ;
		} else if (!strncasecmp(filename, "objn:", 4)) {
			filename += 5;
			filetype = OBJN;
...
// Write an obj file that includes normals
static bool write_objn(TriMesh *mesh, FILE *f)
{
	FPRINTF(f, "# OBJ\n");
	if (!write_verts_asc(mesh, f, "v ", 0, 0, false, 0, ""))
		return false;
    mesh->need_normals();
	if (!write_normals_asc(mesh, f, "vn ", ""))
		return false;
	mesh->need_faces();
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		mesh->faces[i][0]++;
	...
	bool ok = write_faces_normals_asc(mesh, f, "f ", "");
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		mesh->faces[i][0]--;
		mesh->faces[i][1]--;
		mesh->faces[i][2]--;
...
// Write a bunch of normals to an ASCII file
static bool write_normals_asc(TriMesh *mesh, FILE *f,
			    const char *before_norm,
			    const char *after_line)
{
