...
NPRPenStyle::NPRPenStyle()
{
    ...
    _normalTexture = NULL;
    _motionTexture = NULL;
    _interiorTexture = NULL;
    _color = vec(0.1f, 0.1f, 0.1f);
    _opacity = 1.0f;
    _strip_width = 1.0f;
    _length_scale = 1.0f;
    _elision_width = 1.0f;
    _userOffset_Scale = 0.8f;
}
...
void NPRPenStyle::clear()
{
    ...
    if (_motionTexture)
    {
        delete _motionTexture;
        _motionTexture = NULL;
    }

    if (_interiorTexture)
    {
        delete _interiorTexture;
        _interiorTexture = NULL;
    }
    _texture_file = QString();
    _offsetTexture_file = QString();
    _motionTexture_file = QString();
    _interiorTexture_file = QString();
}
...
void NPRPenStyle::copyFrom( const NPRPenStyle& style )
{
    *this = style;
    ...
    _tangentTexture = NULL;
    _normalTexture = NULL;
    setOffsetTexture(_offsetTexture_file);

    _motionTexture = NULL;
    setMotionTexture(_motionTexture_file);

    _interiorTexture = NULL;
    setInteriorTexture(_interiorTexture_file);
}
...
void NPRPenStyle::load( const QDir& style_dir, const QDomElement& element )
{
    ...
    rel_name = element.firstChildElement("offset").text();
    if (!rel_name.isEmpty())
        _offsetTexture_file = style_dir.absoluteFilePath(rel_name);
    rel_name = element.firstChildElement("motionEffect").text();
    if (!rel_name.isEmpty())
        _motionTexture_file = style_dir.absoluteFilePath(rel_name);
    rel_name = element.firstChildElement("interiorTexture").text();
    if (!rel_name.isEmpty())
        _interiorTexture_file = style_dir.absoluteFilePath(rel_name);
    ...
    _userOffset_Scale = element.firstChildElement("offset_scale").text().toFloat();
    ...
    ret = setOffsetTexture(_offsetTexture_file);
    if (!ret)
    {
        _normalTexture = NULL;
        _tangentTexture = NULL;
    ...
    ret = setMotionTexture(_motionTexture_file);
    if (!ret)
    {
        _motionTexture = NULL;
    }

    ret = setInteriorTexture(_interiorTexture_file);
    if (!ret)
    {
        _interiorTexture = NULL;
    }
...
void NPRPenStyle::save( const QDir& style_dir, QDomDocument& doc, QDomElement& element )
{
    ...
    rel_name = style_dir.relativeFilePath(_offsetTexture_file);
    appendTextNode(doc, element, QString("offset"), rel_name);
    rel_name = style_dir.relativeFilePath(_motionTexture_file);
    appendTextNode(doc, element, QString("motionEffect"), rel_name);
    rel_name = style_dir.relativeFilePath(_interiorTexture_file);
    appendTextNode(doc, element, QString("interiorTexture"), rel_name);
    ...
    appendTextNode(doc, element, QString("offset_scale"), QString("%1").arg(_userOffset_Scale));
}

bool NPRPenStyle::setOffsetTexture( const QString& filename)
{
    if (filename.isEmpty())
    {
        return false;
    ...
    if (!QFileInfo(filename).exists())
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not find offset texture \"%1\"").arg(filename));
        return false;
    }

    //load .off file and get the tangent and normal file name
    ...
    QString tangentFile, normalFile;
    header >> _offset_scale;
    header >> tangentFile;
    tangentFile = dir.absoluteFilePath(tangentFile);
    header >> normalFile;
    normalFile = dir.absoluteFilePath(normalFile);

    //load the tangent texture
    GQTexture3D* new_texture = new GQTexture3D();
    if (!new_texture->loadOffsetFile(tangentFile))
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not load offset texture \"%1\" (corrupt texture?)").arg(filename));
        delete new_texture;
        return false;
    }
    if (_tangentTexture)
        delete _tangentTexture;
    _tangentTexture = new_texture;

    //load the tangent texture
    new_texture = new GQTexture3D();
    if (!new_texture->loadOffsetFile(normalFile))
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not load offset texture \"%1\" (corrupt texture?)").arg(filename));
        delete new_texture;
    ...
    if (_normalTexture)
        delete _normalTexture;
    _normalTexture = new_texture;

    _offsetTexture_file = filename;
    //_offset_scale = new_texture->scale();
    return true;
}

bool NPRPenStyle::setMotionTexture( const QString& filename)
{
    if (filename.isEmpty())
    {
        return false;
    ...
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not find motion texture \"%1\"").arg(filename));
        return false;
    }
    ...
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not load motion texture \"%1\" (corrupt texture?)").arg(filename));
        delete new_texture;
        return false;
    }
    ...
    _motionTexture = new_texture;
    _motionTexture_file = filename;

    return true;
}

bool NPRPenStyle::setInteriorTexture( const QString & filename)
{
    if (filename.isEmpty())
    {
        return false;
    ...
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not find interior texture \"%1\"").arg(filename));
        return false;
    }
    ...
    {
        QMessageBox::critical(NULL, "Open Failed",
            QString("Could not load interior texture \"%1\" (corrupt texture?)").arg(filename));
        delete new_texture;
        return false;
    }
    ...
    _interiorTexture = new_texture;
    _interiorTexture_file = filename;
