...
vec4 toonLight( LightingValues vals )
{
  //compute light intensity
  float diffuse = abs(dot( vals.normal, vals.light_vec ));
  
  vec2 texCoord = vec2(max(min(diffuse,0.98),0.1), 1.0-vals.toon_smooth);
  vec4 shade = texture2D(texToon,texCoord);
  float toonIntensity = shade.r;
  
  vec4 objColor = vals.mat_diff_col;
  
  return objColor*(1.0 - vals.toon_factor * (1.0-objColor)*(1.0-toonIntensity));
}
