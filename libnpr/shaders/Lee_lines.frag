// Line drawings via Abstract Sahding, Lee et al. Siggraph 2007
...
uniform sampler2DRect tone_map; // tone map
...
compute_ridge_properties(in vec2 uv, out float max_curv, out float axis_dist, out vec2 axis)
{
   ...
   for(i = 0; i < 3; i++){ // set 3x3 matrix
	 ...
	 T[i*3+j] = texture2DRect(tone_map, neigh_uv).r;
	 n += half_width;
...
vec4 draw_line_pixel(in vec2 center_pix)
{
   ...
   if(is_line == true){ 
      // dark line
      if(max_curv > 0.0){
         col.gb = vec2(-axis.y,axis.x);//tangent

	 // normalized distance
         float dist = length(center_pix - pix2)/half_width; 
	 // center tone value
         float center_val = texture2DRect(tone_map, uv).r;

	 // compute line opacity
         col.r = compute_line_opacity(center_val, max_curv, dist, upper_c, lower_c);
      }
      else{
      // highlight line
...
{ 
    ...
    vec4 color = draw_line_pixel(my_screen);

    float stepsize = 1.0;
    vec2 g = vec2(color.b, -color.g);
