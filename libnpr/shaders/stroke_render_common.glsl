...
vec4 computeInteriorColor( vec3 tex_coord )
{
    float pen = texture3D(vis_focus_texture, tex_coord).r;
    return vec4(pen, pen, pen, 1);
}

vec4 getBasePenColor()
{
    return vis_focus_color;
}
